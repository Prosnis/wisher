...
<template>
    ...
    <button v-if="hasEditPermission" class="profile__button profile__button--addWish" @click="showModalAddCard">
        Добавить
    </button>
    <WiModal ref="modalAddCard">
        <WiCardsAdd :user-img="props.user.photoUrl" :user-name="props.user.displayName" @handleAddWish="handleAddWish" />
    </WiModal>
    <section v-if="userWishList.length" class="wishes__list">
        <div class="whishes__cards">
            <WiCardCreate v-for="wish in userWishList" :key="wish.id" :wish="wish" :user-img="props.user.photoUrl"
